...
namespace rs::core
{
  ...
  // Aliases for hot layout (the original binary format)
  constexpr std::size_t kTrackHotHeaderSize = kTrackHeaderSize;
  constexpr std::size_t kTrackHotHeaderAlignment = kTrackHeaderAlignment;

  /**
   * TrackHotHeader - POD struct for hot track storage.
   * Same as original TrackHeader, renamed for clarity in hot/cold architecture.
   * Layout uses strictly descending member sizes (8→4→2→1) for natural alignment.
