...
namespace ao::winui
{
  ShellWidthClass classifyShellWidth(double const width) noexcept
  {
    if (width < kMediumShellWidth)
    {
      return ShellWidthClass::Narrow;
    }

    if (width < kWideShellWidth)
    {
  ...
  ShellState resolveShellState(ShellMode const mode,
                               double const width,
                               std::optional<bool> const optInspectorRequest) noexcept
  {
    auto const widthClass = classifyShellWidth(width);
