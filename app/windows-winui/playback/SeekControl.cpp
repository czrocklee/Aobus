...
namespace ao::winui
{
  namespace
  {
      ...
      std::wstring_view const name)
    {
      ...
      for (auto index = 0; index < childCount; ++index)
      {
        ...
        if (auto const descendant = findNamedElement(child, name))
        {
          return descendant;
        }
      }
  ...
  {
    auto const upper = _state.duration > std::chrono::milliseconds{0} ? _state.duration : std::chrono::milliseconds{0};
    auto const clamped = std::clamp(elapsed, std::chrono::milliseconds{0}, upper);

    [[maybe_unused]] auto const updating = ScopedBooleanFlag{_updating};
    _slider.Value(static_cast<double>(clamped.count()));
  }
  ...
  std::chrono::milliseconds SeekControl::sliderElapsed() const noexcept
  {
    auto const upper = _interaction.duration() > std::chrono::milliseconds{0}
                         ...
                         : kDefaultMaxRange;
    auto const value = std::clamp(_slider.Value(), 0.0, upper);
  ...
  void SeekControl::applyPointerVisual()
  {
    ...
    if (_thumbElement)
    {
      ...
      if (_pointerOver)
      {
        if (auto const thumb = _thumbElement.try_as<winrt::Microsoft::UI::Xaml::Controls::Control>())
        {
  ...
  void SeekControl::resolveTrackElement()
  {
    ...
    if (auto const thumb = _thumbElement.try_as<winrt::Microsoft::UI::Xaml::Controls::Primitives::Thumb>())
    {
