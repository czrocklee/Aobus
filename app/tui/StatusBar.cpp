...
namespace ao::tui
{
                           ...
                           KeymapPlan const& keymapPlan)
  {
    ...
    auto const navigationInputActive = shell.isNavigationFocused() && state.navigationSearching;

    if (state.hoveredPlaybackMode != nullptr && !state.visualSelectionActive && !navigationInputActive &&
        !shell.isInputActive() && shell.overlay() == Overlay::None)
    {
