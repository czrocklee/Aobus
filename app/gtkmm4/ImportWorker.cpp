...
namespace
{
  std::unique_ptr<rs::tag::File> createTagFileByExtension(std::filesystem::path const& path)
  {
    ...
    if (auto it = CreatorMap.find(ext); it != CreatorMap.end()) {
...
rs::core::TrackRecord ImportWorker::populateRecord(
    rs::tag::Metadata const& metadata,
    ...
    rs::core::ResourceStore::Writer& resourceWriter)
{
  ...
  {
    meta.trackNumber = static_cast<std::uint16_t>(std::get<std::int64_t>(metadata.get(rs::tag::MetaField::TrackNumber)));
  }

  // Total tracks
  if (!rs::tag::isNull(metadata.get(rs::tag::MetaField::TotalTracks)))
  {
    meta.totalTracks = static_cast<std::uint16_t>(std::get<std::int64_t>(metadata.get(rs::tag::MetaField::TotalTracks)));
  }
  ...
  {
    prop.fileSize = std::filesystem::file_size(path);
  }
