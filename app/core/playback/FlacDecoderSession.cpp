...
namespace app::core::playback
{
  ...
  FlacDecoderSession::FlacDecoderSession(StreamFormat outputFormat)
  ...
  {
    close();
    _impl->currentPath = filePath;
    _impl->eof = false;
    _impl->nextFrameIndex = 0;

    auto const initStatus = FLAC__stream_decoder_init_file(_impl->decoder,
  ...
  std::optional<PcmBlock> FlacDecoderSession::readNextBlock()
  {
    ...
    block.endOfStream = _impl->eof && (FLAC__stream_decoder_get_state(_impl->decoder) == FLAC__STREAM_DECODER_END_OF_STREAM);
  ...
  FLAC__StreamDecoderWriteStatus FlacDecoderSession::Impl::writeCallback(const FLAC__StreamDecoder* /*decoder*/,
                                                                         const FLAC__Frame* frame,
                                                                         const FLAC__int32* const buffer[],
                                                                         void* clientData)
  {
    ...
    // We only support 16-bit and 24-bit for now as output
    // Simplification: output matches source bit depth unless requested otherwise
  ...
  void FlacDecoderSession::Impl::metadataCallback(const FLAC__StreamDecoder* /*decoder*/,
                                                  const FLAC__StreamMetadata* metadata,
                                                  void* clientData)
  {
    auto* impl = static_cast<Impl*>(clientData);
    ...
    if (metadata->type == FLAC__METADATA_TYPE_STREAMINFO)
    {
      ...
      if (impl->requestedOutput.bitDepth != 0) impl->info.outputFormat.bitDepth = impl->requestedOutput.bitDepth;

      if (metadata->data.stream_info.sample_rate > 0)
      {
        impl->info.durationMs = static_cast<std::uint32_t>(
          metadata->data.stream_info.total_samples * 1000 / metadata->data.stream_info.sample_rate);
      }
      impl->info.isLossy = false;
    }
  }

  void FlacDecoderSession::Impl::errorCallback(const FLAC__StreamDecoder* /*decoder*/,
                                               FLAC__StreamDecoderErrorStatus status,
                                               void* clientData)
  {
    auto* impl = static_cast<Impl*>(clientData);
