...
namespace ao::rt
{
  ...
  {
  public:
    CompletionService(library::MusicLibrary& library, LibraryMutationService& mutation);
    ~CompletionService();
  ...
  private:
    ...
    // every LibraryMutationService::tracksMutated emit is marshalled onto the main thread before the
