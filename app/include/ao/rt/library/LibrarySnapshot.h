...
namespace ao::rt
{
  ...
  // pays for only one transaction. Obtain one via Library::snapshot().
  class [[nodiscard]] LibrarySnapshot final
  {
  public:
    LibrarySnapshot(LibrarySnapshot&&) noexcept;
    LibrarySnapshot& operator=(LibrarySnapshot&&) noexcept;
    LibrarySnapshot(LibrarySnapshot const&) = delete;
  ...
  private:
    ...
    explicit LibrarySnapshot(library::MusicLibrary const& library);
  };
} // namespace ao::rt
