...
namespace ao::uimodel
{
   ...
   * (`editMode` + `onNodeMoved`). Owned by the frontend's shell controller and
   * borrowed by the per-build context. StatefulComponentState retains a pointer
   * to this object so it can write component runtime state back after a build
   * call returns, which is why it must outlive any single build.
   */
  struct LayoutRuntimeState final
  {
    std::string activePresetId{};
    LayoutComponentStateDocument componentState{};
    LayoutComponentStateStore* componentStateStore = nullptr;
