...
{
  class CoverArtByteCache final
  {
  public:
    static constexpr std::size_t kDefaultMaximumEntries = 128;

    explicit CoverArtByteCache(std::size_t maximumEntries = kDefaultMaximumEntries);

    void reset();
    bool store(ResourceId resourceId, std::vector<std::byte> bytes);
  ...
  {
  public:
    static constexpr std::size_t kDefaultMaximumEntries = CoverArtByteCache::kDefaultMaximumEntries;
    ...
    std::size_t cachedCount() const noexcept { return _cache.cachedCount(); }
    std::size_t maximumEntries() const noexcept { return _cache.maximumEntries(); }

  private:
    ResourceId _selectedResourceId{kInvalidResourceId};
    std::uint64_t _generation = 0;
    CoverArtByteCache _cache;
  };
} // namespace ao::uimodel
