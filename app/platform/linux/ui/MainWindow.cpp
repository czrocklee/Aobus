...
void MainWindow::buildPageForStoredList(rs::core::ListId listId, rs::core::ListView const& view)
{
  ...
  if (view.isSmart())
  {
    ...
    auto filtered = std::make_unique<app::core::model::FilteredTrackIdList>(*sourceList, *_musicLibrary, *_smartListEngine);
  ...
  auto exporter = std::make_unique<app::services::PlaylistExporter>(*membershipList, *_rowDataProvider, _musicLibrary->rootPath(), playlistPath);
...
void MainWindow::refreshPlaybackBar()
{
  ...
  if (previousState == app::core::playback::TransportState::Playing && snapshot.state == app::core::playback::TransportState::Idle)
  {
