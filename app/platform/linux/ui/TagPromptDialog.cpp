...
void TagPromptDialog::stageAddTag(std::string tag)
{
  ...
  it->second.pending = it->second.membershipCount == _selectionCount ? PendingTagChange::Keep : PendingTagChange::AddToAll;
  _tagEntry.set_text("");
...
void TagPromptDialog::rebuildTagRows()
{
  ...
  for (auto const& tag : visibleTags)
  {
    ...
    if (state.pending == PendingTagChange::AddToAll)
    {
      ...
      {
        statusText = "currently on " + std::to_string(state.membershipCount) + " of " + std::to_string(_selectionCount) +
                     ", add to all on save";
      }
    }
