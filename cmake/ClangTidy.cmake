...
#
# Usage:
#   rockstudio_apply_clang_tidy(<target> STRICT)   # lib, tool, app targets
#   rockstudio_apply_clang_tidy(<target> RELAXED)  # test targets

unset(CLANG_TIDY_EXECUTABLE CACHE)
unset(CLANG_TIDY_EXECUTABLE)

if(ROCKSTUDIO_ENABLE_CLANG_TIDY)
  find_program(CLANG_TIDY_EXECUTABLE NAMES clang-tidy REQUIRED)
endif()

if(CLANG_TIDY_EXECUTABLE)
  # Build GCC system include paths for clang-tidy
  set(_RS_CLANG_TIDY_EXTRA_ARGS)
  execute_process(
  ...
  foreach(CXX_INCLUDE_DIR IN LISTS CXX_INCLUDE_DIRS)
    ...
    if(IS_DIRECTORY "${CXX_INCLUDE_DIR}")
      list(APPEND _RS_CLANG_TIDY_EXTRA_ARGS --extra-arg-before=-isystem${CXX_INCLUDE_DIR})
    endif()
  endforeach()

  # Strict checks (lib, tool, app)
  set(_RS_CLANG_TIDY_STRICT_CHECKS
    "-checks=-*,bugprone-*,performance-*,cppcoreguidelines-*,-cppcoreguidelines-avoid-magic-numbers,-cppcoreguidelines-avoid-const-or-ref-data-members,modernize-!-trailing-return,readability-*,portability-*,-clang-diagnostic-*,-bugprone-easily-swappable-parameters,-cppcoreguidelines-pro-bounds-pointer-arithmetic,-readability-convert-member-functions-to-static,-readability-static-definition-in-anonymous-namespace,-clang-diagnostic-note"
  )
  ...
  # Relaxed checks (test)
  set(_RS_CLANG_TIDY_RELAXED_CHECKS "-checks=-*,performance-avoid-endl")

  # Header filters (portable — uses CMAKE_SOURCE_DIR instead of hardcoded path)
  set(_RS_CLANG_TIDY_STRICT_FILTER "'${CMAKE_SOURCE_DIR}/(lib|tool|app)/.*'")
  set(_RS_CLANG_TIDY_RELAXED_FILTER "'${CMAKE_SOURCE_DIR}/test/.*'")

  message(STATUS "clang-tidy found: ${CLANG_TIDY_EXECUTABLE}")
  message(STATUS "clang-tidy strict checks for lib/tool/app, relaxed for test/")
endif()

# Apply clang-tidy to a target with the given mode (STRICT or RELAXED).
function(rockstudio_apply_clang_tidy target mode)
  ...
  if(mode STREQUAL "STRICT")
    set_target_properties(${target} PROPERTIES
      CXX_CLANG_TIDY "${CLANG_TIDY_EXECUTABLE};${_RS_CLANG_TIDY_STRICT_CHECKS};--config=${_RS_CLANG_TIDY_STRICT_CONFIG};-header-filter=${_RS_CLANG_TIDY_STRICT_FILTER};${_RS_CLANG_TIDY_EXTRA_ARGS}"
    )
  elseif(mode STREQUAL "RELAXED")
    set_target_properties(${target} PROPERTIES
      CXX_CLANG_TIDY "${CLANG_TIDY_EXECUTABLE};${_RS_CLANG_TIDY_RELAXED_CHECKS};-header-filter=${_RS_CLANG_TIDY_RELAXED_FILTER};${_RS_CLANG_TIDY_EXTRA_ARGS}"
    )
  else()
    message(FATAL_ERROR "rockstudio_apply_clang_tidy: unknown mode '${mode}' (expected STRICT or RELAXED)")
  endif()
endfunction()
