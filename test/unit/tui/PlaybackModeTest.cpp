...
namespace ao::tui::test
{
            ...
            "[tui][unit][playback-mode]")
  {
    ...
    auto mode = rt::PlaybackSuccessionSnapshot{};
    auto state = StatusBarViewState{
      .terminalColumns = 120, .navigationSearching = true, .shell = &shell, .hoveredPlaybackMode = &mode};
    auto render = [&] { return renderElement(statusBar(catalog, state, defaultKeymapPlan()), 120, 1).text; };
    auto const searching = render();
    CHECK(searching.contains("Tab tracks"));
    CHECK(searching.contains("/ search · Esc clear"));
    CHECK_FALSE(searching.contains("Click for"));

    state.navigationSearching = false;
    auto const browsing = render();
    CHECK(browsing.contains("In order · Click for Repeat all"));
