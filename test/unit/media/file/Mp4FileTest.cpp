...
namespace ao::media::file::mp4::test
{
  ...
  namespace
  {
    std::vector<std::uint8_t> makeTextMetadataAtom(char const* atomType, std::string_view text)
    {
                                               ...
                                               std::vector<std::uint8_t> const& sampleEntryExtensions = {})
    {
      ...
      {
        auto const atom = makeTextMetadataAtom(type, text);
                                    ...
                                    std::optional<std::uint8_t> optBaseline = std::nullopt)
    {
      ...
      return field == MovementField::Number ? metadata.number(NumberField::MovementNumber)
                                            : metadata.number(NumberField::MovementTotal);
    }
  ...
  TEST_CASE("MP4 File - parses a complete tagged file", "[media][unit][mp4][file]")
  {
  ...
  TEST_CASE("MP4 File - reads metadata from an extended-size semantic atom", "[media][regression][mp4]")
  {
    auto const title = ao::test::mp4::makeExtendedFromCompactAtom(makeTextMetadataAtom("\xA9"
                                                                                       "nam",
                                                                                       "Extended Title"));
    auto const ilst = ao::test::mp4::makeAtom("ilst", title);
    auto metaBody = std::vector<std::uint8_t>{0, 0, 0, 0};
    metaBody.insert(metaBody.end(), ilst.begin(), ilst.end());
    auto const meta = ao::test::mp4::makeAtom("meta", metaBody);
    auto const udta = ao::test::mp4::makeAtom("udta", meta);
    auto const track = ao::test::mp4::makeAudioTrackAtom("alac");
    auto moovBody = track;
    moovBody.insert(moovBody.end(), udta.begin(), udta.end());

    auto data = ao::test::mp4::makeAtom("moov", moovBody);
    auto const mdat = ao::test::mp4::makeAtom("mdat", {1});
    ...
    CHECK(content.text(TextField::Title) == "Extended Title");
  }

  TEST_CASE("MP4 File - emits real fixture tag fields", "[media][unit][mp4][file]")
  {
  ...
  TEST_CASE("MP4 File - maps freeform classical names case-insensitively", "[media][unit][mp4][file]")
  {
    auto ilstChildren = std::vector<std::uint8_t>{};
            ...
            "[media][regression][mp4][file]")
  {
    ...
    {
      auto data = ao::test::mp4::makeAtom("free", {0xAA});
      auto const mdat = ao::test::mp4::makeEndOfFileAtom("mdat", payload);
      auto const expectedOffset = data.size() + sizeof(AtomLayout);
      data.insert(data.end(), mdat.begin(), mdat.end());
      auto const temp = TempFile{data, ".m4a"};
      auto const file = File{temp.path};
      auto const rangeResult = file.audioPayload();
  ...
  TEST_CASE("MP4 File - audio payload signature ignores non-mdat atoms", "[media][unit][mp4][file]")
  {
  ...
  TEST_CASE("MP4 File - parses movement integer metadata", "[media][unit][mp4][file]")
  {
  ...
  TEST_CASE("MP4 File - parses track and disc number pairs", "[media][unit][mp4][file]")
  {
    SECTION("Accepts a 32-byte disk atom")
    {
      ...
      CHECK(content.number(NumberField::DiscNumber) == 2);
      CHECK(content.number(NumberField::DiscTotal) == 5);
    }
    ...
    SECTION("Accepts a 30-byte trkn atom")
    {
      ...
      CHECK(content.number(NumberField::TrackNumber) == 2);
      CHECK(content.number(NumberField::TrackTotal) == 4);
    }
  }
