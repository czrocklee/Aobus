...
{
  TEST_CASE("TrackColumnLayoutCodec - round-trip uses stable field ids", "[uimodel][unit][track-column-layout]")
  {
    ...
    auto const document = encodeTrackColumnLayout(state);
    ...
    auto const decoded = decodeTrackColumnLayout(*document);
  ...
  TEST_CASE("TrackColumnLayoutCodec - rejects an invalid document as one object",
            "[uimodel][unit][track-column-layout]")
  {
    ...
    {
      document.version = 2;
      auto const result = decodeTrackColumnLayout(document);
  ...
  TEST_CASE("TrackColumnLayoutCodec - refuses to encode noncanonical live state",
            "[uimodel][unit][track-column-layout]")
  {
    auto state = TrackColumnLayoutState{};
    ...
    auto const result = encodeTrackColumnLayout(state);
