...
{
  TEST_CASE("buildListTreeProjection projects nested list rows", "[uimodel][unit][list][tree]")
  {
  ...
  TEST_CASE("buildListTreeProjection attaches invalid parents under all tracks", "[uimodel][unit][list][tree]")
  {
  ...
  TEST_CASE("buildListTreeProjection orders children by list id", "[uimodel][unit][list][tree]")
  {
    auto const parentId = ListId{2};
    auto const projection = buildListTreeProjection(std::vector{
