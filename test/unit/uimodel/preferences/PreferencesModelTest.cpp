...
{
  TEST_CASE("PreferencesModel - theme changes persist and apply selected theme", "[uimodel][unit][preferences]")
  {
    ...
    auto model = PreferencesModel{initial,
  ...
  TEST_CASE("PreferencesModel - output changes persist engine-confirmed selection", "[uimodel][unit][preferences]")
  {
  ...
  TEST_CASE("PreferencesModel - layout preset changes persist for the next layout load", "[uimodel][unit][preferences]")
  {
    auto optPersisted = std::optional<rt::AppPrefsState>{};
    auto initial = rt::AppPrefsState{};
    initial.lastThemePreset = "modern";
