...
namespace
{
  ...
  struct TestIdTag
  {};
  using TestId = ao::utility::TaggedInteger<std::uint32_t, TestIdTag>;
  ...
  struct TestStringTag
  {};
  using TestStringId = ao::utility::StrongId<TestStringTag>;
...
namespace ao::rt::test
{
  ...
  TEST_CASE("ConfigStore - edge cases", "[app][runtime][config]")
  {
    ...
    SECTION("Loading from non-existent file returns NotFound in ReadOnly mode")
    {
      ...
      REQUIRE(!result);
      CHECK(result.error().code == ao::Error::Code::NotFound);
