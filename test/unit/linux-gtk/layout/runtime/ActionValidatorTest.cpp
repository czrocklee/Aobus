...
namespace ao::gtk::layout::test
{
  TEST_CASE("ActionValidator", "[layout][unit][runtime]")
  {
    ...
    registry.registerAction(
      ActionDescriptor{.id = "test.action", .label = "Test Action", .category = "Test", .capabilities = ActionCapability::None},
      [](auto&) {});
    registry.registerAction(
      ActionDescriptor{.id = "test.anchored", .label = "Anchored", .category = "Test", .capabilities = ActionCapability::RequiresAnchor},
      [](auto&) {});
    ...
    SECTION("Validates good document")
    {
      ...
      auto diagnostics = validateActions(doc, compRegistry, registry);
