...
TEST_CASE("TrackListPresentation: Artist mapping", "[app][runtime][presentation]")
{
  ...
  std::vector<TrackSortField> expectedSort = {TrackSortField::Artist, TrackSortField::Album, TrackSortField::DiscNumber, TrackSortField::TrackNumber, TrackSortField::Title};
...
TEST_CASE("TrackListPresentation: Album mapping", "[app][runtime][presentation]")
{
  ...
  std::vector<TrackSortField> expectedSort = {TrackSortField::AlbumArtist, TrackSortField::Album, TrackSortField::DiscNumber, TrackSortField::TrackNumber, TrackSortField::Title};
  REQUIRE(snapshot.effectiveSortBy.size() == expectedSort.size());
  for (size_t i = 0; i < expectedSort.size(); ++i) {
    REQUIRE(snapshot.effectiveSortBy[i].field == expectedSort[i]);
  }

  std::vector<TrackSortField> expectedRedundant = {TrackSortField::Artist, TrackSortField::Album, TrackSortField::AlbumArtist};
  REQUIRE(snapshot.redundantFields == expectedRedundant);
}
...
TEST_CASE("TrackListPresentation: Genre mapping", "[app][runtime][presentation]")
{
  ...
  std::vector<TrackSortField> expectedSort = {TrackSortField::Genre, TrackSortField::Artist, TrackSortField::Album, TrackSortField::DiscNumber, TrackSortField::TrackNumber, TrackSortField::Title};
  REQUIRE(snapshot.effectiveSortBy.size() == expectedSort.size());
  for (size_t i = 0; i < expectedSort.size(); ++i) {
    REQUIRE(snapshot.effectiveSortBy[i].field == expectedSort[i]);
  }
...
TEST_CASE("TrackListPresentation: Composer mapping", "[app][runtime][presentation]")
{
  ...
  std::vector<TrackSortField> expectedSort = {TrackSortField::Composer, TrackSortField::Artist, TrackSortField::Album, TrackSortField::DiscNumber, TrackSortField::TrackNumber, TrackSortField::Title};
  REQUIRE(snapshot.effectiveSortBy.size() == expectedSort.size());
  for (size_t i = 0; i < expectedSort.size(); ++i) {
    REQUIRE(snapshot.effectiveSortBy[i].field == expectedSort[i]);
  }
...
TEST_CASE("TrackListPresentation: Work mapping", "[app][runtime][presentation]")
{
  ...
  std::vector<TrackSortField> expectedSort = {TrackSortField::Work, TrackSortField::Artist, TrackSortField::Album, TrackSortField::DiscNumber, TrackSortField::TrackNumber, TrackSortField::Title};
  REQUIRE(snapshot.effectiveSortBy.size() == expectedSort.size());
  for (size_t i = 0; i < expectedSort.size(); ++i) {
    REQUIRE(snapshot.effectiveSortBy[i].field == expectedSort[i]);
  }
...
TEST_CASE("TrackListPresentation: Year mapping", "[app][runtime][presentation]")
{
  ...
  std::vector<TrackSortField> expectedSort = {TrackSortField::Year, TrackSortField::Artist, TrackSortField::Album, TrackSortField::DiscNumber, TrackSortField::TrackNumber, TrackSortField::Title};
  REQUIRE(snapshot.effectiveSortBy.size() == expectedSort.size());
  for (size_t i = 0; i < expectedSort.size(); ++i) {
    REQUIRE(snapshot.effectiveSortBy[i].field == expectedSort[i]);
  }
