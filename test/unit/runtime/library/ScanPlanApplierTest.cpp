...
namespace ao::rt::test
{
  ...
  namespace
  {
    // Counts the failures the applier pushes through its failure callback.
  ...
  TEST_CASE("ScanPlanApplier - initial scans process new files", "[runtime][unit][library][scan]")
  {
    ...
    auto executor = ScanPlanApplier{ml, std::move(plan), nullptr, counts.callback()};
  ...
  TEST_CASE("ScanPlanApplier - deferred new scans write pending audio identity", "[runtime][unit][library][scan]")
  {
    auto const temp = ao::test::TempDir{};
    auto const musicRoot = std::filesystem::path{temp.path()} / "music";
    std::filesystem::create_directories(musicRoot);
    ...
    auto counts = FailureCounts{};
    auto executor = ScanPlanApplier{ml,
                                    std::move(plan),
                                    nullptr,
                                    counts.callback(),
  ...
  TEST_CASE("ScanPlanApplier - defer policy still uses cached new identity", "[runtime][unit][library][scan]")
  {
    auto const temp = ao::test::TempDir{};
    auto const musicRoot = std::filesystem::path{temp.path()} / "music";
    std::filesystem::create_directories(musicRoot);
  ...
  TEST_CASE("ScanPlanApplier - reports fingerprint progress while hashing audio payload",
            "[runtime][unit][library][scan]")
  {
    ...
    auto counts = FailureCounts{};
    auto executor = ScanPlanApplier{ml, std::move(plan), std::move(progress), counts.callback()};
    auto runResult = executor.run();
    REQUIRE(runResult);
  ...
  TEST_CASE("ScanPlanApplier - cancellation aborts partial scan transaction", "[runtime][unit][library][scan]")
  {
    auto const temp = ao::test::TempDir{};
    auto const musicRoot = std::filesystem::path{temp.path()} / "music";
    std::filesystem::create_directories(musicRoot);
  ...
  TEST_CASE("ScanPlanApplier - cancellation aborts between fingerprint chunks", "[runtime][unit][library][scan]")
  {
    auto const temp = ao::test::TempDir{};
    auto const musicRoot = std::filesystem::path{temp.path()} / "music";
    std::filesystem::create_directories(musicRoot);
  ...
  TEST_CASE("ScanPlanApplier - cancellation clears failures from the aborted transaction",
            "[runtime][unit][library][scan]")
  {
    auto const temp = ao::test::TempDir{};
    auto const musicRoot = std::filesystem::path{temp.path()} / "music";
  ...
  TEST_CASE("ScanPlanApplier - skips unchanged files", "[runtime][unit][library][scan]")
  {
    auto const temp = ao::test::TempDir{};
    auto const musicRoot = std::filesystem::path{temp.path()} / "music";
    std::filesystem::create_directories(musicRoot);
    ...
    // First scan to populate the manifest
    {
      auto scanner = ScanPlanBuilder{ml};
      auto plan = scanner.buildPlan().value();
      auto executor = ScanPlanApplier{ml, std::move(plan), nullptr, nullptr};
      auto runResult = executor.run();
      REQUIRE(runResult);
    }
  ...
  TEST_CASE("ScanPlanApplier - updates changed files", "[runtime][unit][library][scan]")
  {
    auto const temp = ao::test::TempDir{};
    auto const musicRoot = std::filesystem::path{temp.path()} / "music";
    std::filesystem::create_directories(musicRoot);
  ...
  TEST_CASE("ScanPlanApplier - updates manifest status for missing files", "[runtime][unit][library][scan]")
  {
    auto const temp = ao::test::TempDir{};
    auto const musicRoot = std::filesystem::path{temp.path()} / "music";
    std::filesystem::create_directories(musicRoot);
  ...
  TEST_CASE("ScanPlanApplier - relinks moved files while preserving DB-owned curation",
            "[runtime][unit][library][scan]")
  {
    auto const temp = ao::test::TempDir{};
    auto const musicRoot = std::filesystem::path{temp.path()} / "music";
  ...
  TEST_CASE("ScanPlanApplier - rejects moved files whose live identity changed after planning",
            "[runtime][unit][library][scan]")
  {
    auto const temp = ao::test::TempDir{};
    auto const musicRoot = std::filesystem::path{temp.path()} / "music";
  ...
  TEST_CASE("ScanPlanApplier - aborts the transaction when moved manifest removal fails",
            "[runtime][unit][library][scan]")
  {
    auto const temp = ao::test::TempDir{};
    auto const musicRoot = std::filesystem::path{temp.path()} / "music";
  ...
  TEST_CASE("ScanPlanApplier - reports corrupted file failures", "[runtime][unit][library][scan]")
  {
    auto const temp = ao::test::TempDir{};
    auto const musicRoot = std::filesystem::path{temp.path()} / "music";
    std::filesystem::create_directories(musicRoot);
  ...
  TEST_CASE("ScanPlanApplier - propagates unexpected process exceptions", "[runtime][unit][library][scan]")
  {
    auto const temp = ao::test::TempDir{};
    auto const musicRoot = std::filesystem::path{temp.path()} / "music";
    std::filesystem::create_directories(musicRoot);
    ...
    auto executor = ScanPlanApplier{ml, std::move(plan), std::move(thrower), counts.callback()};

    REQUIRE_THROWS_AS(std::ignore = executor.run(), std::runtime_error);
    CHECK(counts.failed == 0);
  }

  TEST_CASE("ScanPlanApplier - ignores non-decodable files omitted from the plan", "[runtime][unit][library][scan]")
  {
    auto const temp = ao::test::TempDir{};
    auto const musicRoot = std::filesystem::path{temp.path()} / "music";
    std::filesystem::create_directories(musicRoot);
