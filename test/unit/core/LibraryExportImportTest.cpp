...
TEST_CASE("Library Export/Import Cycle", "[app][core][yaml]")
{
  ...
  rs::core::LibraryExporter exporter(ml1);
  REQUIRE_NOTHROW(exporter.exportToYaml(yamlPath, rs::core::ExportMode::Full));
  ...
  rs::core::LibraryImporter importer(ml2);
...
library:
  ...
  rs::core::LibraryImporter importer(ml);
  REQUIRE_NOTHROW(importer.importFromYaml(yamlPath));

  {
    ...
    std::optional<rs::core::ListView> parent;
    std::optional<rs::core::ListView> child;
    ...
    REQUIRE(parent->parentId() == rs::core::ListId{0});
