...
This reference owns the exact Windows-side surfaces of the version 1 layout language: the component and action ids the Windows schema registers, the layout fields it accepts and rejects, the native element each component constructs, the `styleKey` and `surface` extensions, and the two built-in preset documents.

It uses the same unversioned `LayoutDocument` version 1 language as the [layout document reference](../shell/layout-document.md). The Windows schema is a deliberately narrower identity set than the [GTK layout schema](../shell/layout-schema.md): the two share no preset and no component construction, but a type both shells present takes its identity and shared properties from the [shared component vocabulary](../shell/component-vocabulary.md) rather than being described twice.
...
Menus, reveal-current-track, and column editing remain native behavior of the component that owns them and have no layout-schema action id.
...
No component here accepts `secondaryLongPressAction`. Windows raises one holding sequence per press regardless of which button started it, so the binder cannot tell a secondary hold from a primary one and refuses the slot. Offering it in the schema would let a document validate and then be rejected in full while being built, so the schema does not offer it; a test holds every schema entry to that. GTK's soul button does accept the slot, because GDK distinguishes the two.

`playback.soulButton` has no `primaryAction` default, because that slot is the soul's own play/pause gesture: the soul runs the transport when the document leaves the slot alone, and presents playback without driving it when the document names an action there. Nothing else in the schema puts a native gesture and an action slot on the same event, so this is the only component where authoring an action takes behavior away.
...
`cssClasses` is a GTK extension and is rejected. A component may also declare per-child layout fields in its schema entry; no built-in Windows component currently does.
...
| Reason | Condition |
|---|---|
| `UnknownComponentType` | The type is not registered in the Windows schema |
...
| `ChildCountBelowMinimum` | Fewer children than the schema entry or the authored presentation requires |
| `ChildCountAboveMaximum` | More children than the schema entry or the authored presentation allows |
...
| `UnknownAction` | A bound or defaulted action id outside the Windows action schema |
...
The schema ids are internal shipped identities, not a user-authoring surface: this adoption supports only the two built-in presets, with no Windows layout editor, user-authored preset, or migration format.
...
- [`LayoutSchema.cpp`](../../../app/windows-winui/layout/LayoutSchema.cpp) owns registered component and action ids, presentation-dependent element and child rules, and required ids.
...
- [`LayoutSchemaTest.cpp`](../../../test/unit/winui/layout/LayoutSchemaTest.cpp), [`PlacementPlanTest.cpp`](../../../test/unit/winui/layout/PlacementPlanTest.cpp), [`StyleLookupTest.cpp`](../../../test/unit/winui/layout/StyleLookupTest.cpp), and [`ElementKindTest.cpp`](../../../test/unit/winui/layout/ElementKindTest.cpp) cover the schema, placement mapping, style planning, and element lattice.
