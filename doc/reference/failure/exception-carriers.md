...
| `ao::query::detail::QueryException` | Query implementation detail; thrown by query and format compilation helpers. | `QueryCompiler::compile()` or `FormatCompiler::compile()`. | Carried `Error` as `Result`. |
