...
| Editor widget | `preferences/ShortcutEditorWidget` hosted by `PreferencesWindow` | `linux-gtk` |
...
## Preferences Keyboard Page

**Edit → Preferences… → Keyboard** hosts `ShortcutEditorWidget`, backed by the same
`KeymapModel`. It enumerates the shortcut-eligible actions from the `ActionCatalog`
— every action *except* those that require a widget anchor or present a menu
(`RequiresAnchor` / `PresentsMenu`), since a global accelerator has no surface to
anchor against — grouped by their declared category. The catalog supplies the
human-readable labels; the chords come from the effective keymap.
...
`PreferencesWindow` is owned at application scope. Closing it hides the window, and
`app.preferences` re-presents the same instance. The shortcut editor is refreshed
from the active main window's action catalog and the current persisted keymap each
time Preferences is presented. The transient key-capture popup defers its own
destruction to an idle callback whose connection is cancelled in the editor's
destructor, so a pending teardown never touches freed state.

## Not yet covered

- App-scoped shortcut customization. `Ctrl+,` is a fixed `app.preferences`
  accelerator and is intentionally not part of the layout-action keymap.
