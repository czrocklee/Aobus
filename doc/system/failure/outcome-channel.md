...
It delegates the exact `Error` fields and code inventory to the [error value reference](error.md), cross-layer ownership to the [failure and reporting architecture](README.md), executor and cancellation mechanics to the [runtime execution architecture](../execution/README.md), and subsystem-specific recovery behavior to the owning subsystem specification.

## Code boundary

The top-level layer direction is defined by the [system architecture](../overview.md), and failure ownership is refined by the [failure and reporting architecture](README.md).
...
Exact category tokens, macro call shapes, diagnostics, sink lifetime, truncation, and reentrancy behavior belong to the [fatal facility reference](fatal.md).
...
- The [exception carrier reference](exception-carriers.md) owns the exact carrier whitelist and catch owners.
...
- [Notification feed specification](notification-feed.md)
