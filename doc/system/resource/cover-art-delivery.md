...
This specification defines current behavior for describing cover content by digest, attaching ordered cover references, selecting a primary cover, reading verified bytes through runtime from a derived cache or a carrier media file, and delivering cover art through GTK, WinUI, AppKit, TUI, MPRIS, and CLI.
...
This contract spans the **Core libraries**, **application runtime**, **UIModel**, and **frontend** layers from the [system architecture](../overview.md), under the [resource delivery architecture](README.md).
...
- **Shared no-cover placeholder**: a UIModel-style presentation shown by a visible GTK or WinUI cover slot whose resource id is invalid. AppKit's native system-note fallback and the TUI's compact absence line are separate frontend degradation presentations.
...
- A shared no-cover placeholder never acts as a stored resource, decoded cover result, MPRIS URL, SMTC artwork payload, or MediaPlayer artwork payload.
- In GTK and WinUI cover slots, a valid resource id takes precedence over the shared no-cover placeholder, including while its bytes are pending or after delivery or decode failure. AppKit may draw its independent native fallback and the TUI may render its compact absence line while no decoded or transformed artwork is available.
...
### Current playback artwork

The shared playback snapshot publishes the current track's live primary cover
alongside its title, artist and album. Committed cover replacement, removal and
track deletion reach Now Playing consumers in GTK, WinUI, TUI and system media
adapters through the normal playback subscription. TUI artwork itself follows
its selected detail track. Metadata refresh and missing-track fallback belong
to [playback application commits](../playback/application-commit.md#live-now-playing-metadata).

### GTK full-size image

The GTK Now Playing component selects the track, primary cover and placeholder identity from the shared playback snapshot.
It has no separate library-change subscription or track read. Same-track cover edits and removals arrive through that snapshot.
Idle transport clears the displayed track, cover and placeholder identity even if the snapshot retains now-playing metadata.
...
### AppKit and MediaPlayer

`LibrarySession` keeps separate request interests and identities for the selected-track and playing-track covers.
Replacing either identity cancels its prior interest and clears the corresponding `ResourceBytes` before requesting the new id from the session's shared runtime cache.
Current completion publishes owned encoded bytes in the native presentation snapshot; `AobusTrackInspector` and `AobusPlaybackBar` decode them into `NSImage` on the main thread.
Their `AobusArtworkView` draws its independent system-note fallback whenever no native image is available, including pending or failed valid-resource delivery; this is not one of the UIModel placeholder styles.

The session-owned `MediaPlayerAdapter` has an independent artwork interest over the same encoded-byte cache.
It records the replacement id and clears old artwork before requesting, then accepts completion only while it is not retired and the id is still current.
A synchronous cache hit publishes the updated Now Playing metadata and artwork inside the callback, so the outer snapshot path does not publish a duplicate update.
`MPMediaItemArtwork` retains an image copy in its request handler rather than borrowing the adapter or session.
Artwork completion preserves the existing transport clock anchor.
...
| --- | ---: | --- |
| Resource bytes read for GTK, WinUI, AppKit, TUI, or MPRIS | 32 MiB | `ValueTooLarge`, adapted to no decoded image/URL/artwork |
...
AppKit publishes selected and playing encoded bytes only for the current identities; its native views and MediaPlayer adapter decode independently. Absent or stale results publish no decoded cover or MediaPlayer artwork, while an image-less native `AobusArtworkView` draws its own system-note fallback.
TUI shows its compact absence line while delivery is pending and when the resource is absent, over-budget, or undecodable; this line is not a shared graphical no-cover placeholder and does not reserve an artwork panel.
...
- [Resource delivery architecture](README.md)
