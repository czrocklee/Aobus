...
It assigns ownership for route evidence, graph composition, analysis, runtime publication, platform-neutral presentation, and frontend adaptation from Core audio through GTK, TUI, WinUI, and AppKit.
...
Those facts belong to the [audio quality specification](quality-analysis.md) and [surface reference](quality-values.md).

## System context

Audio quality is a focused cross-cutting path through the layers established by the [system architecture](../overview.md), within the broader ownership graph of the [playback architecture](README.md):
...
| Toolkit/terminal adaptation | Frontends | Frontend-local | `app/linux-gtk/playback/`, `app/linux-gtk/css/`, `app/tui/`, `app/windows-winui/playback/`, and `app/macos-appkit/` |
...
AppKit's native Soul control consumes the same `AobusSoulViewModel` through its library session; this does not imply detailed pipeline-panel parity.
Each implemented surface consumes UIModel values; no frontend owns a second severity table, headline policy, or Soul aura interpretation.

## Boundaries and dependency direction

- Core audio quality types cannot depend on runtime, UIModel, or a frontend.
...
- Frontends may adapt UIModel categories and labels but cannot reinterpret finding kinds into different severities.
...
provider graph callback
  ...
  -> UIModel presentation
  -> frontend rendering (GTK/TUI/WinUI surfaces and AppKit Soul)
```
...
Player represents absent evidence as an empty/incomplete graph and readiness state; the behavioral consequences belong to the [quality specification](quality-analysis.md).
...
- [`AobusSoul.cpp`](../../../app/linux-gtk/app/AobusSoul.cpp), [`SoulButton.cpp`](../../../app/tui/SoulButton.cpp), [`AobusSoulControl.cpp`](../../../app/windows-winui/playback/AobusSoulControl.cpp), and AppKit [`SoulButton.mm`](../../../app/macos-appkit/SoulButton.mm) adapt shared Soul presentation. AppKit [`LibrarySession.cpp`](../../../app/macos-appkit/LibrarySession.cpp) owns its shared view model and subscription.
...
- [Audio quality analysis specification](quality-analysis.md)
