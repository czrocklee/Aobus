...
Raw/effective rank semantics and writer transactions belong to [track sources](../library/track-source.md) and [library mutation](../library/mutation.md).
...
- `Busy` also leaves an otherwise current session available for explicit retry.
- `Applied`, `Stale`, `Unavailable`, and operational `Error` outcomes consume the session; a later move starts from a new binding.
...
Writer statuses are `Applied`, `NoOp`, `Busy`, `Stale`, or `Unavailable`; a submission may instead return an operational `Error`.
...
An operational `Error` invalidates the session and is reported rather than retried against the same binding.
