...
GTK keeps one pending deletion for five seconds. A newer same-key edit on overlapping targets, selection replacement, timeout, successful/no-op replay, or terminal replay failure clears it; `Busy` is non-terminal and leaves the pending action available for retry.
...
Custom-metadata undo returns a stale, unavailable, or operational terminal failure, clears the expired action, and the undo bar publishes that failure as an error notification. A `Busy` result reports temporary resource contention without clearing the pending undo.
