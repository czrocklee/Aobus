...
Concrete music-library keys, records, alignment, and versioning belong to the [library database reference](../../reference/library/storage/database.md); shared failure-channel selection belongs to the [outcome channel specification](../failure/outcome-channel.md); application-level ownership belongs to the [persistence and managed-state](README.md) and [library](../library/structure.md) architectures.

## Code boundary

The [system architecture](../overview.md) places the LMDB adapter in the core-library layer.
...
- A **byte-key database** is a `ByteKeyDatabase` token whose key domain is non-empty byte strings and whose public reader and writer accept byte-span keys.
...
- Non-empty byte-span key operations exist only on `ByteKeyDatabase` readers and writers; a caller cannot select or query a runtime key kind.
...
| Active write | Snapshot reads and staged writes are available. | Create, update, append, delete, clear, commit, and abort are available. | `commit()`, `abort()`, destruction, or move-out makes the transaction terminal. A `detail::TransactionFailure` requires the owner to take the abort transition. |
...
A `ByteKeyDatabase` accepts non-empty byte-span keys and iterates in LMDB byte order.
...
The marker does not cross into runtime, CLI, or frontend code, and the owning contract forbids continuing or committing the failed root even though the native handle remains active until the owner aborts it.
