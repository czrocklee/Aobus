...
The [persistence and managed-state architecture](README.md) assigns grouped file mechanics to `ConfigStore`, reusable YAML and file-replacement mechanics to Core, schema and semantic policy to payload owners, and path selection to composition roots.
...
- One non-copyable, non-movable store owns its optional backing location, live tree, parser input buffer, and parser callback state.
...
- A returned serialization, candidate-emission, replacement, or removal failure leaves both the live document and backing file unchanged under the atomic-replacement contract. An exception raised before replacement likewise leaves both unchanged, but the store does not translate it.
- When configured, emitted replacement bytes must fit the same file-byte ceiling before atomic replacement.
- In a located store, successful save or effective removal replaces the complete backing file and only then installs the matching candidate as the live document.
...
- A store constructed with no location initially reads as an empty document and never touches the filesystem. Generic store saves update that instance's live document and report success having written nothing; no-location use never resolves an empty path relative to the working directory.
...
The store has only lazy-load state. In a located store, every public mutation either completes its whole-file replacement or leaves the live document unchanged; no-location stores install only an in-memory candidate as described below.
...
A composition root may construct `ConfigStore::NoLocation` when it intentionally continues without a usable persistent path.
That includes a platform resolver naming no home or profile location and may include a frontend failing to prepare an otherwise resolved directory.
The composition root must use this mode only for state whose loss is survivable, and it reports the degraded session where the cause is still known.
`ConfigStore` itself neither discovers a path nor diagnoses why none was supplied.

The first operation establishes an empty top-level mapping without filesystem inspection.
Every group initially reads as absent, so a load leaves its seeded value unchanged.
Generic `save()`, `saveTogether()`, and effective `removeGroup()` operations still use a private complete-document candidate.
A successful save installs that candidate as the instance's live document and reports success without emitting bytes; later loads through the same instance therefore observe its saved groups.
Candidate serialization failures and exceptions retain the same isolation as for a located store: a returned serialization failure or an exception before installation leaves the live document unchanged, and exceptions are not translated.
No operation resolves a backing path against the process working directory, inspects the filesystem, or invokes atomic replacement.
Destruction still performs no save, and a new no-location instance starts empty.

Generic save success is mechanism policy, not a requirement that every semantic save workflow accept volatile persistence.
`hasLocation()` lets an owner impose a stricter contract before calling the store.
Explicit save acknowledgement may therefore differ from a passive checkpoint. The [application managed-state surface](../../reference/persistence/application-config.md#frontend-policy-without-a-persistent-application-location) owns those frontend-specific decisions, including TUI Settings rejection.
...
The `NoLocation` constructor takes no path at all and is `ReadWrite`; its absence of a path is fixed for the store lifetime.
The mode and optional file-byte ceiling are likewise fixed for a located store's lifetime.

### Lazy initialization

For a located store, the first initializing operation inspects the backing path.
...
For a located store, the operation proceeds in this order:
...
| An exception escapes deserialization | The exception propagates; assignment has not occurred, so the seeded value remains unchanged. |
| An exception escapes serialization, candidate assembly, or emission | The exception propagates; replacement has not been attempted, so the live document and backing file remain unchanged. |
...
Schema and candidate-processing exceptions are not translated by `ConfigStore`; callers that permit throwing schemas or allocation failures must contain them at their own operation boundary.
...
Every effective mutation of a located store replaces one complete YAML file; group updates are not independently written. A no-location store has no durable output.
...
- [`ConfigStore.h`](../../../app/include/ao/rt/ConfigStore.h) owns the public modes, the `NoLocation` constructor, `ConfigSchema` concept, `ConfigWrite` descriptors, explicit load/save templates, candidate serialization and isolation, schema-exception propagation, and returned-error context.
...
- Frontend composition owns when a session supplies no location. [`PlatformDirectories.h`](../../../include/ao/utility/PlatformDirectories.h) owns one input to that decision: whether the platform names an application directory.
...
- [`ConfigStoreTest.cpp`](../../../test/unit/runtime/ConfigStoreTest.cpp) protects explicit schema round trips, presence results, seeded values, byte ceilings, failed-deserialize isolation, multi-group atomicity, propagation and isolation of schema exceptions, replacement failure, temporary-string ownership, removal, read-only behavior, and the no-location session's same-instance live groups and lack of filesystem output.
...
- [Persistence and managed-state architecture](README.md)
