...
status: current
domain: development
summary: Defines identifier, type, vocabulary, file, and support-code naming policy.
---
# Naming conventions
...
- Public `uimodel` declarations use the flat `ao::uimodel` namespace described
  in `doc/development/uimodel-organization.md`; do not add feature-specific public
...
- Optional existence-check syntax is covered in `doc/development/coding-style.md`
...
- Do not encode test-only access in names such as `*ForTest()`. Testability
  seam rules live in `doc/development/test/fixture-and-helper.md`.
