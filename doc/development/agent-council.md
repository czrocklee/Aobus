---
id: development.agent-council
type: development
status: current
domain: development
summary: Defines the advisory agent-council workflow, governance, outputs, and failure handling.
---
# Aobus agent council

`aobus-council` and `config/agent-council.yaml` are the production agent mechanism.
