...
- [RFC 0010: Versioned presentation state](0010-versioned-presentation-state.md) is implemented with stable textual ids, layered versioned codecs, strict candidate decoding, and no legacy numeric migration.
...
- [RFC 0015: Fail-closed grouped configuration transactions](0015-fail-closed-config-store.md) is rejected: candidate decoding and one-shot complete-document saves closed the destructive paths without blocked-store recovery, generations, or commit receipts.
