...
- [RFC 0005: Coherent playback application boundary](0005-coherent-playback-boundary.md) is accepted with one public `PlaybackService`, one coherent application snapshot, a single command authority, and worker-safe preparation; its serialized configuration-writer stage was not adopted because playback-session persistence now uses best-effort event-driven checkpoints without dirty revisions or retry scheduling.
...
- [RFC 0011: Executor-affine reporting feed](0011-executor-affine-reporting-feed.md) is implemented as one immutable snapshot-update stream with callback-executor confinement, bounded authoritative retention, and coherent activity projection.
...
- [RFC 0016: Coherent workspace transactions](0016-coherent-workspace-transactions.md) is implemented with narrow command results, one complete revisioned snapshot stream, copy-then-commit history and restore, and AppRuntime-owned album reveal.
...
- [RFC 0030: Structured presentation vocabulary](0030-structured-presentation-vocabulary.md) is implemented with stable semantic values, structured missing/completion/audio/report inputs, one typed UIModel English catalog, and frontend-owned native rendering.
