...
- [RFC 0005: Coherent playback application boundary](0005-coherent-playback-boundary.md) is accepted with one public `PlaybackService`, one revisioned application snapshot, a single commit authority, and worker-safe preparation; its serialized configuration-writer stage was not adopted because playback-session persistence now uses best-effort event-driven checkpoints without dirty revisions or retry scheduling.
