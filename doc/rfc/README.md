...
- [RFC 0005: Coherent playback application boundary](0005-coherent-playback-boundary.md) is accepted with one public `PlaybackService`, one revisioned application snapshot, a single commit authority, a general serialized configuration writer for shared stores, and worker-safe preparation, resolving its open questions on writer scope, single route-retry, per-command completion, source-context starts, and retained typed events.
