...
TUI navigation labels, overlay titles and hints, command-palette metadata, help copy, playback/output empty states, library navigation/filter status, recoverable errors, and remaining accessibility-oriented status copy resolve through `chromeText`, TUI-local formatting functions, or direct canonical ids; presentation-navigation qualifiers use direct ids through `MessageCatalog`.
