...
WinUI shares coalesced encoded-byte loads and an at-most-128-entry cache among presenters bound to one library runtime.
...
A coordinator-owned loader coalesces equal resource requests and shares its bounded encoded-byte cache among group-heading and Inspector presenters; playback uses a loader bound to the playback runtime so a retained retiring library remains valid.
A current cached or loaded payload replaces the image through the native XAML decoder.
...
GTK pixbufs, WinUI encoded-byte entries, TUI transforms, and MPRIS files are derived process/cache artifacts and can be discarded.
...
- [`WindowsCoverArtLoader`](../../../app/windows-winui/image/WindowsCoverArtLoader.h) and [`CoverArtPresenter`](../../../app/windows-winui/image/CoverArtPresenter.h) own WinUI delivery.
