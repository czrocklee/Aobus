...
installs the pinned official CPython 3.13.13 distribution under the local Aobus
state directory, and creates a checkout-specific virtual environment. Ruff
0.15.14, mypy 1.20.1, and their locked dependencies are installed in that
environment. The base Python is shared across checkouts; the virtual environment
...
`%LOCALAPPDATA%\Aobus\tools\python\3.13.13\python.exe` with the default state
root.
...
`clang+llvm` 22.1.8 Windows development archive, verifies its pinned SHA-256,
and extracts about 3.8 GiB below
...
cmake -S . --preset windows-tidy -B C:\local\aobus-build\windows-tidy ^
  -DAOBUS_LLVM_SDK_ROOT=C:/toolchains/clang+llvm-22.1.8-x86_64-pc-windows-msvc
```
...
```bat
set "OLD_SDK=%CD%\out\toolchains\llvm-22.1.8-x86_64-windows-msvc"
set "NEW_SDK=%LOCALAPPDATA%\Aobus\cache\llvm\toolchains\llvm-22.1.8-x86_64-windows-msvc"
