...
`default` is intentionally the normal development loop. On Linux, the TUI, CLI, integration, council, tooling,
and lint suites take longer, so they are included only when selected directly or through `all`. The Windows
`all` group contains core, TUI, and integration because the normal Windows test presets do not build GTK,
CLI, the council tool, or the lint integration suite. Native `ao.bat tidy` uses a separate `windows-tidy`
preset that builds the self-contained `AobusClangTidy.exe`. The Ruff/mypy-backed tooling gate remains in the Linux `all` group,
where those tools are pinned by the Nix environment instead of being taken from an ambient Windows `PATH`.
...
`--no-build` applies uniformly. Catch2 executables and the native lint artifact must already exist in the selected
build tree; tooling tests never need a CMake build. `--path`, compiler, and sanitizer options select the same
tree for C++ and lint integration suites. On Windows, a build-enabled test command configures the matching
`windows-tui-*-tests` preset automatically; application-only and test-enabled trees are intentionally separate.

Tooling tests are exposed on Linux as `./ao test --tooling`, not as a separate top-level command. Their normal
