...
Observer handlers are ordinary callables behind the owning signal boundary: contract-fulfilling handlers observe the committed snapshot, while an escaping exception enters AO fatal handling at that boundary and provides no later-handler guarantee.
