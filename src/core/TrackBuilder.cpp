...
TrackBuilder::TrackBuilder(TrackRecord record) : _record{std::move(record)} {}

MetadataBuilder TrackBuilder::metadata() { return MetadataBuilder{_record}; }
PropertyBuilder TrackBuilder::property() { return PropertyBuilder{_record}; }
TagsBuilder TrackBuilder::tags() { return TagsBuilder{_record}; }
CustomBuilder TrackBuilder::custom() { return CustomBuilder{_record}; }

//=============================================================================
// TagsBuilder
//=============================================================================

TagsBuilder& TagsBuilder::add(std::string tagName)
{
  _record.tags.names.push_back(std::move(tagName));
  return *this;
}

TagsBuilder& TagsBuilder::remove(std::string tagName)
{
  std::erase(_record.tags.names, tagName);
  return *this;
}
...
CustomBuilder& CustomBuilder::add(std::string key, std::string value)
{
  _record.custom.pairs.emplace_back(std::move(key), std::move(value));
  return *this;
}

CustomBuilder& CustomBuilder::remove(std::string key)
{
  std::erase_if(_record.custom.pairs, [&key](auto const& pair) { return pair.first == key; });
  return *this;
}
...
TrackHotHeader TrackBuilder::buildHotHeader(DictionaryStore& dict, lmdb::WriteTransaction& txn, std::span<DictionaryId const> tagIds) const
{
...
TrackColdHeader TrackBuilder::buildColdHeader(std::size_t customCount, std::uint16_t uriOffset, std::uint16_t uriLen) const
{
...
std::vector<std::byte> TrackBuilder::serializeHot(DictionaryStore& dict, lmdb::WriteTransaction& txn) const
{
  ...
  for (auto const& name : _record.tags.names)
  {
...
std::vector<std::byte> TrackBuilder::serializeCold(DictionaryStore& dict, lmdb::WriteTransaction& txn) const
{
  ...
  // Build header
  std::uint16_t uriOffset = static_cast<std::uint16_t>(
    (sizeof(TrackColdHeader) + entryCount * kEntrySize) + totalValueSize);
...
std::pair<std::vector<std::byte>, std::vector<std::byte>> TrackBuilder::serialize(
