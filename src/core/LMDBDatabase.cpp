...
namespace rs::core
{
  ...
  Reader LMDBDatabase::reader(LMDBReadTransaction& txn) const { return Reader{_impl->dbi, txn._txn}; }

  Writer LMDBDatabase::writer(LMDBWriteTransaction& txn) { return Writer{_impl->dbi, txn._txn}; }
  ...
  boost::asio::const_buffer Reader::operator[](std::uint64_t id) const
  {
    ...
    return _dbi.get(_txn, lmdb::ptr_to_sv(&id), value) ? boost::asio::buffer(value.data(), value.size())
  ...
  Reader::Iterator::Iterator(const Iterator& other) : _cursor{nullptr}, _value{other._value}
  {
    if (other._cursor != nullptr)
    {
      ...
      std::string_view key = lmdb::ptr_to_sv(&_value.first);
  ...
  void Reader::Iterator::increment()
  {
    ...
    {
      _value.first = lmdb::from_sv<std::uint64_t>(key);
  ...
  {
    std::string_view key;
    _lastId = _cursor.get(key, MDB_LAST) ? lmdb::from_sv<std::uint64_t>(key) : 0;
  }
  ...
  namespace
  {
    ...
    {
      auto key = lmdb::ptr_to_sv(&id);
      auto value = toStrView(data);
      cursor.put(key, value, flags);
      // lmdb::cursor_put(cursor.handle(), lmdb::ptr_to_sv(&id), toStrView(data), flags);
    ...
    {
      ::MDB_val key{sizeof(id), reinterpret_cast<char*>(id)};
      ::MDB_val val{size, nullptr};
      lmdb::cursor_put(cursor.handle(), &key, &val, flags | MDB_RESERVE);
  ...
  {
    auto id = ++_lastId;
    return {id, reserve(_cursor, ++_lastId, size, MDB_NOOVERWRITE | MDB_APPEND)};
  }

  const void* Writer::update(std::uint64_t id, boost::asio::const_buffer data) { return put(_cursor, id, data, 0); }

  bool Writer::del(std::uint64_t id) { return _dbi.del(_txn, lmdb::ptr_to_sv(&id)); }
