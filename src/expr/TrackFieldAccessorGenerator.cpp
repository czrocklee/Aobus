...
namespace rs::expr
{
  ...
  {
    using CppCodeWriter = rs::gen::CppCodeWriter;
  ...
  private:
    ...
    void generateName2IdFunction(CppCodeWriter& writer, const Fields& fields)
    {
      ...
      writer | "  };"
             ...
             | "  {"
             | "    RS_THROW_FORMAT(core::Exception, \"Invalid field name : {} for track \", name);"
             | "  }"
             | "  else"
             | "  {"
    ...
    void generateTrackAccessor(CppCodeWriter& writer, const Fields& fields, const char* name)
    {
      ...
      writer | "static rs::core::DataValue get(const rs::fbs::Track* track, int fieldId)"
             | "{"
             | "  switch (fieldId)"
             | "  {";
      ...
      for (auto field : fields)
      {
        ...
        if (field->type()->base_type() == ::reflection::String)
        {
          ...
          writer ( "  auto val = track->%1%()->%2%();", name, field->name()->c_str() )
                 | "  return val == nullptr ? rs::core::DataValue{} : std::string_view{val->c_str(), val->size()};"
                 | "}";
        }
    ...
    void generateTrackTAccessor(CppCodeWriter& writer, const Fields& fields, const char* name)
    {
      ...
      writer | "static rs::core::DataValue get(const rs::fbs::TrackT& track, int fieldId)"
             | "{"
             | "  switch (fieldId)"
             | "  {";
      ...
      for (auto field : fields)
      {
        ...
        {
          writer ( "case %1%:", field->id() )
                 ( "  return track.%1%->%2%.empty() ? rs::core::DataValue{} : std::string_view{track.%1%->%2%};", name, field->name()->c_str() );
        }
        else if (flatbuffers::IsInteger(field->type()->base_type()))
        {
          writer ( "case %1%:", field->id() )
