...
namespace
{
  ...
  const x3::symbols<Operator> relationalOperator{{{"=", Operator::Equal},
                                                  {"~", Operator::Like},
                                                  {"<", Operator::Less},
                                                  {"<=", Operator::LessEqual},
                                                  {">", Operator::Greater},
  ...
  const auto logicalOr_def = logicalAnd >> -as<BinaryExpression::Operation>(keyword(logicalOrOperator) >> logicalOr);
  const auto logicalAnd_def = relational >> -as<BinaryExpression::Operation>(keyword(logicalAndOperator) >> logicalAnd);
  const auto relational_def = arithmetic >> -as<BinaryExpression::Operation>(relationalOperator >> relational);
  const auto arithmetic_def = primary >>
  ...
  const auto logicalNot_def = keyword(logicalNotOperator) >> logicalOr;
