...
#include <rs/core/Tag.h>
#include <rs/core/Custom.h>
#include <rs/core/Exception.h>
#include <rs/expr/gen/TrackFieldAccessor.h>
    ...
    public:
      ...
      DataValue operator()(const VariableExpression& variable)
      {
        if constexpr (std::is_pointer_v<T>)
        {
          if (track == nullptr)
          {
            RS_THROW(core::Exception, "Invalid access to variable type where track is not provided");
          }
        }
        ...
        switch (variable.type)
        {
          ...
          case VariableType::Tag: return rs::core::tag(track, variable.name);
          case VariableType::Custom: return rs::core::custom(track, variable.name);
          default: return {};
        }
      }
      ...
      {
        return std::visit(utility::makeVisitor([](const auto& val) { return DataValue{val}; }), constant);
      }
