...
namespace
{
  using namespace rs::gen;
  namespace bpo = boost::program_options;
  namespace bfs = boost::filesystem;

  bool getFields(const std::string& schemaContent, std::string_view name, Fields& fields)
  {
    ::flatbuffers::Verifier verifier(
      reinterpret_cast<const std::uint8_t*>(schemaContent.c_str()), schemaContent.length());
    ...
    if (root->name()->string_view() != name)
    {
      std::cerr << "Invalid root table, " << name << " expected" << std::endl;
      return false;
    }

    fields.assign(root->fields()->begin(), root->fields()->end());
  ...
  try
  {
    ...
    for (const auto& gp : generatorPaths)
    {
      ...
      generators.push_back(
        boost::dll::import_symbol<Generator>(path, "generator", boost::dll::load_mode::append_decorations));
    }
