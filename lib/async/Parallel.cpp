...
#pragma GCC diagnostic ignored "-Wmaybe-uninitialized"
  ...
  Task<> whenAll(Runtime* runtime, std::vector<Task<>> tasks)
  {
    ...
    using SpawnOperation =
      decltype(boost::asio::co_spawn(runtime->workerPool(), std::move(tasks.front()), boost::asio::deferred));
    auto operations = std::vector<SpawnOperation>{};
    operations.reserve(tasks.size());

    for (auto& task : tasks)
    {
      operations.push_back(boost::asio::co_spawn(runtime->workerPool(), std::move(task), boost::asio::deferred));
    }
